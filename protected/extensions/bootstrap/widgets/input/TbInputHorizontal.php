...
class TbInputHorizontal extends TbInput
{
	...
	protected function autoComplete(){//todo 未完成
		echo $this->getLabel();
		echo '<div class="controls">';

		$data = $this->data;
		if(!empty($data['extField'])){
			foreach($data['extField'] as $f) CHtml::activeHiddenField($this->model,$f);
		}
		$source = is_array($data['data'])?json_decode($data['data']):'js:function(data, callback){
			jQuery.getJSON("'.$data['data'].'",data, callback);
		}';
		$this->widget('zii.widgets.jui.CJuiAutoComplete', [
			'name' => 'display_'.$this->attribute,
			'source' => $source,
			'options'=>[
